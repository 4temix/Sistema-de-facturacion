...
const NARROW_MQ = `(max-width: ${LG_BREAKPOINT - 1}px)`;
...
function useMobileProfileMenuLayout(
  ...
  const update = useCallback(() => {
    ...
    const menuW = Math.min(MENU_MAX_W, vw - 2 * VIEWPORT_PAD);
    const idealLeft = rect.left + rect.width / 2 - menuW / 2;
    const left = Math.max(
      VIEWPORT_PAD,
      Math.min(idealLeft, vw - VIEWPORT_PAD - menuW),
    );
    setLayout({ top: rect.bottom + 8, left, width: menuW });
  }, [triggerRef]);
...
function menuPanelClassName() {
  return "flex flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark";
}
...
export default function UserDropdown() {
  ...
  const mobileLayout = useMobileProfileMenuLayout(isOpen, buttonRef);
  ...
  const showMobilePortal =
    isOpen && narrow && mobileLayout && typeof document !== "undefined";
  const showDesktopDropdown = isOpen && !narrow;
    ...
    <div className="relative">
      ...
      {showMobilePortal &&
        createPortal(
          <div
            ref={mobileMenuRef}
            className={`${menuPanelClassName()} z-[99999]`}
            style={{
              position: "fixed",
              top: mobileLayout.top,
